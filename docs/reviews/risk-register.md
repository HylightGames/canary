...
| R-35 | Plugins / Marketplace | The Tier A/B split addresses trust (how much a plugin can touch), not the orthogonal question of what happens when multiple plugins claim the same subsystem-replacement role (e.g. two Tier B plugins both trying to be *the* physics backend) — no conflict-resolution rule exists | Low | Open | [Review #3 triage](../decisions/2026-09-review-3-triage.md#11--extension--replacement--override-as-distinct-plugin-concepts), point 11; not urgent before Tier B subsystem-replacement plugins are a real multi-plugin scenario (Era 6), named now while cheap |
| R-36 | Core runtime | `Subsystem` (`engine/canary-core/src/subsystem.rs`) has exactly three lifecycle hooks — `init`, `tick`, `shutdown` — with no `pause`/`resume`/`reload`/`replace` concept; hot reload, editor play-mode, and live collaboration will all eventually need richer lifecycle semantics than this | Low | Open | [Review #3 triage](../decisions/2026-09-review-3-triage.md#12--subsystem-lifecycle-before-subsystems-multiply), point 12; timed to whenever scripting hot-reload (Era 3) or live collaboration (Era 4) actually starts, not before |
