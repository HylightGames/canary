...
| R-03 | Plugin ABI | Tier B vtable has no version field or extension mechanism; future additions risk silent ABI breakage | Critical | Open | [ADR 0009](../decisions/architecture-decision-records/0009-plugin-abi-versioning-and-extensibility.md); small code change, cost rises the longer it's deferred |
| R-04 | ECS | Component identity (`TypeId`) has no meaning across the Rust/WASM/cross-language boundary | Critical | Open | [ADR 0010](../decisions/architecture-decision-records/0010-component-identity-across-language-boundary.md) (Proposed); needs prototyping alongside the archetype ECS migration |
| R-05 | Rust architecture | ECS component storage lacks `Send + Sync` bounds; contradicts the target parallel job-system design | High | Open | Review §2.2; small code change, recommended before any external component types exist |
...
| R-12 | ECS | Entity generation counter (`u32`) can wrap on a long-lived, hot-recycled slot; wraparound is currently silent | Medium | Open | Review §4.2; low probability, high cost, currently undocumented as a tradeoff |
| R-13 | ECS | Change detection is required by 3+ subsystems but designed in none | Medium | Open | Review §4.3; design alongside the archetype migration, not after |
...
| R-18 | Build system | `xtask check` silently diverges from what CI actually gates on (skips clippy) | Medium | Open | Review §8.2; small code change, deferred per this review's own scope |
