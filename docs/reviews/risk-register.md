...
| R-04 | ECS | Component identity (`TypeId`) has no meaning across the Rust/WASM/cross-language boundary | Critical | Mitigated | Fixed for `v0.0.2`/`v0.0.3`: [ADR 0010](../decisions/architecture-decision-records/0010-component-identity-across-language-boundary.md) (Accepted); `CanaryComponent`/`World::register_component`/`type_id_for_schema` prototyped in `v0.0.2`, and the actual Tier A consumer landed in `v0.0.3` (`engine/canary-plugin-api/src/component_value.rs`'s `CodecRegistry`, wired through `wit/plugin.wit`'s `ecs-read`/`ecs-write`) — no longer future work |
...
| R-14 | Language independence | WIT interface versioning strategy undecided | Medium | Open | Review §5.2; resolve before Tier A lands. **Not resolved**: `wit/plugin.wit` shipped with `v0.0.3` (`package canary:plugin@0.1.0`) carrying *a* version number, but no actual decided strategy for what triggers a bump or what compatibility guarantee it implies. Landed without this, honestly — worth resolving before a second WIT interface makes the gap harder to retrofit |
| R-15 | Language independence | Host/component call-boundary performance for bulk ECS data is asserted, not measured | Medium | Open | Review §5.3; benchmark early in Tier A work before API surface assumes a cost model. **Not resolved**: `v0.0.3`'s Tier A tests prove correctness (a real value round-trips through a real `World`) but never measure call-boundary cost — the AOT test explicitly proves the execution *path* works, not that it's fast, per its own doc comment. The `get`/`set` API surface therefore still doesn't have a validated cost model behind it |
...
| R-33 | ECS | `World::query_changed_since` (`v0.0.2`) detects component *mutation* but has no signal for component *removal* or entity *destruction* — a removed row simply stops being reported, with nothing left to indicate it was removed rather than never having existed. Networking replication genuinely needs this (a despawned entity has to disappear client-side) | High | Open | Named in [ADR 0014](../decisions/architecture-decision-records/0014-change-detection-as-shared-primitive.md)'s "Known gaps" section during external review of that ADR, before Era 4 was designed against the assumption this was already solved. Likely shape: a durable removal/destruction log or tombstone mechanism, consulted alongside `query_changed_since` rather than folded into it — not yet designed |
| R-34 | Plugins | Tier A's `HostState` (`engine/canary-plugin-api/src/tier_a.rs`) owns a `canary_ecs::World` by value rather than safely lending scoped access to one already running elsewhere — fine for `v0.0.3`'s tests (each constructs its own throwaway `World`), not yet a design for a real engine loop where systems and Tier A plugins would need to share one live `World` | Medium | Open | Named directly in `tier_a.rs`'s and `docs/architecture/plugin-system.md`'s doc comments as this release shipped, not discovered later. Belongs with whatever the parallel job-stealing scheduler (still unbuilt) settles for *every* system's `World` access, not something to solve ad hoc for Tier A specifically |
| R-35 | Plugins / Marketplace | The Tier A/B split addresses trust (how much a plugin can touch), not the orthogonal question of what happens when multiple plugins claim the same subsystem-replacement role (e.g. two Tier B plugins both trying to be *the* physics backend) — no conflict-resolution rule exists | Low | Open | [Review #3 triage](../reviews/triage/2026-09-review-3-triage.md#11--extension--replacement--override-as-distinct-plugin-concepts), point 11; not urgent before Tier B subsystem-replacement plugins are a real multi-plugin scenario (Era 6), named now while cheap |
| R-36 | Core runtime | `Subsystem` (`engine/canary-core/src/subsystem.rs`) has exactly three lifecycle hooks — `init`, `tick`, `shutdown` — with no `pause`/`resume`/`reload`/`replace` concept; hot reload, editor play-mode, and live collaboration will all eventually need richer lifecycle semantics than this | Low | Open | [Review #3 triage](../reviews/triage/2026-09-review-3-triage.md#12--subsystem-lifecycle-before-subsystems-multiply), point 12; timed to whenever scripting hot-reload (Era 3) or live collaboration (Era 4) actually starts, not before |
