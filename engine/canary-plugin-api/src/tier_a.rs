...
impl WasmPluginLoader {
    /// Creates a loader with the Component Model enabled.
    ...
    /// [`Capability::ReadEcsWorld`] has no way to even *reach* that
    ...
    fn instantiate(
        ...
        // No other capability has a corresponding interface in
        // `wit/plugin.wit` yet (see the module docs), so there is
        // nothing further to conditionally link for `WriteEcsWorld`,
        // `Filesystem`, or `Network` at this stage.

        let mut store = Store::new(&self.engine, HostState { world });
...
mod tests {
    ...
    fn granting_read_ecs_world_lets_the_component_read_real_world_state() {
        let loader = WasmPluginLoader::new().expect("engine setup should not fail");
